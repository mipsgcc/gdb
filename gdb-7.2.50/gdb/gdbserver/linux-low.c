...
linux_wait_for_event_1 (ptid_t ptid, int *wstat, int options)
{
  ...
  while (1)
	  ...
	  && event_child->stop_expected)
	{
      ...
      if (WIFSTOPPED (*wstat)
	  && WSTOPSIG (*wstat) == SIGINT
	  && event_child->sigint_expected == -1)
	{
	  if (debug_threads)
	    fprintf (stderr, "Expected SIGINT.\n");

	  event_child->sigint_expected = 0;
	  linux_resume_one_lwp (event_child,
				event_child->stepping, 0, NULL);
	  continue;
	}
...
/* Cancel or ignore a SIGINT we have queued ourselves.  */

static int
cancel_sigint_callback (struct inferior_list_entry *entry, void *data)
{
  struct lwp_info *lp = (struct lwp_info *) entry;
  struct thread_info *thread = get_lwp_thread (lp);
  struct lwp_info *event_lp = data;

  /* Leave the LWP that has been elected to report an event alone.  */
  if (lp == event_lp)
    return 0;

  /* If we've seen the pending SIGINT, consume it; otherwise, set the
  ...
  if (thread->last_status.kind == TARGET_WAITKIND_IGNORE
      && lp->sigint_expected)
    {
	  ...
	  && WIFSTOPPED (lp->status_pending)
	  && WSTOPSIG (lp->status_pending) == SIGINT)
	{
	  fprintf (stderr,
		   "CSC: cancelling SIGINT on %s\n",
		   target_pid_to_str (ptid_of (lp)));

	  lp->status_pending_p = 0;
	  lp->sigint_expected = 0;
	}
      else
	{
	  /* Still pending; ignore the next time we see it.  */
	  lp->sigint_expected = -1;
	}
    }
...
Check if we're already there.\n",
  ...
  if (!non_stop && !stabilizing_threads)
    {
      /* Stop handing interrupt requests (leading to SIGINTs) queued
	 in already stopped processes.  Need to do this before the
	 cancel_sigint_callback calls below, as otherwise, an
	 asynchronous SIGIO signal could be handled, and a SIGINT
	 queued in the process after we'd just tried to cancel pending
	 SIGINTs.  */
      disable_async_io ();
      ...
      if (ptid_equal (ptid, minus_one_ptid))
	{
	 ...
	 why.  */
      ...
      /* Cancel any pending interrupt request (SIGINT) that may have
	 gone through just after EVENT_CHILD had stopped and reported
	 some other event.  The RSP docs say: "Interrupts received
	 while the program is stopped are discarded.".  */
      find_inferior (&all_lwps, cancel_sigint_callback, event_child);

      if (event_child->sigint_expected)
	{
	  if (WSTOPSIG (w) == SIGINT)
	    {
	      if (debug_threads)
		fprintf (stderr, "SIGINT queued, and it has arrived.\n");
	      event_child->sigint_expected = 0;
	    }
	  else
	    {
	      if (debug_threads)
		fprintf (stderr, "SIGINT queued, but it hasn't arrived yet.\n");
	      event_child->sigint_expected = -1;
	    }
	}
...
linux_request_interrupt (void)
{
  ...
  /* Note: we should maybe be doing `kill(-pg, SIGINT)' to each the
     process group we're debugging instead.  */

  if (!ptid_equal (cont_thread, null_ptid)
      && !ptid_equal (cont_thread, minus_one_ptid))
    lwp = get_thread_lwp (current_inferior);
  else
    lwp = find_lwp_pid (pid_to_ptid (signal_pid));
