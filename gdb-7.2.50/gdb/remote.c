...
remote_wait_as (ptid_t ptid, struct target_waitstatus *status, int options)
{
 ...
 again:
  ...
  /* The RSP docs say: "Interrupts received while the program is
     stopped are discarded.".  Then if the user typed ctrl-c, and we
     got back a signal other than SIGINT, make sure we still cancel
     the current command.  This is useful if the user wants to
     interrupt some lengthy single-stepping operation.  */
